...
const native_detection = @import("engine_test").core.native_detection_record;
const native_retry = @import("engine_test").core.native_retry;
...
test "native maintenance: source sequence receipt and cursor commit or roll back together" {
    ...
    const policy = @import("engine_test").core.native_retry.Policy{ .maxretry = 2, .window_us = 1000, .duration = .{ .finite_us = 1000 }, .max_subjects = 8 };
...
test "native maintenance: retired retries preserve counters and original clocks on reactivation" {
    ...
    try f.store.admitRetry(identity.jail, generation, policy);
    const subject = @import("engine_test").core.native_detection_record.Subject{ .v4 = .{ 192, 0, 2, 91 } };
    ...
    const detection = @import("engine_test").core.native_detection_record;
    const outcome = @import("engine_test").core.source_time_policy.Result{ .eligible = .{ .timestamp = .{ .us = 600 }, .receipt = .{ .us = 600 }, .origin = .event, .original = .{ .us = 600 } } };
...
fn candidateRecord(id: durable.ReceiptIdentity, revision: u64, at: i64, policy: @import("engine_test").core.native_retry.Policy) !durable.Record {
    const detection = @import("engine_test").core.native_detection_record;
    var record = recordFor(id, revision);
...
test "native maintenance: inclusive retry window pins detail and retirement until after endpoint" {
    ...
    defer f.deinit();
    try setupCleanup(&f);
    const policy = @import("engine_test").core.native_retry.Policy{ .maxretry = 2, .window_us = 100, .duration = .{ .finite_us = 100 }, .max_subjects = 8 };
    try f.store.admitRetry(identity.jail, generation, policy);
    _ = try f.store.beginReceipt(identity, .{ .us = 100 }, 0);
