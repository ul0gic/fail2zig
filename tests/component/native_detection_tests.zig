...
const projection = @import("engine_test").config.native_file_detection;
const config = @import("engine_test").config.native;
const registry = @import("engine_test").filters.registry;
const parser = @import("engine_test").core.parser;
const policy = @import("engine_test").core.source_time_policy;
const processor = @import("engine_test").core.native_source_processor;
...
const portsentry = @import("engine_test").filters.portsentry;
const t = std.testing;
comptime {
...
test "native detection: native decoding and time preparation feed a staged candidate" {
    ...
    defer plan.deinit(t.allocator);
    const now = try @import("engine_test").core.native_time.parse(.iso8601, "2026-09-12T10:00:01Z", .{});
    var scratch: [2048]u8 = undefined;
...
test "native detection: actual file consumers commit typed results and recover pending evidence in every codec" {
    for (std.enums.values(text.Encoding)) |encoding| {
        ...
        clock.now += 1_000_000;
        {
            ...
            try t.expectEqual(@import("engine_test").core.native_detection_record.Kind.candidate, found.kind);
        ...
        clock.now += 601_000_000;
        {
            ...
            const source = &session.sources.sources.items[0];
            const found = (try store.nativeDetection("ssh", source.source_id, null)).?;
            try t.expectEqual(@import("engine_test").core.native_detection_record.Kind.time_excluded, found.kind);
...
test "native detection: consumer returns detached typed exclusion and address outcomes" {
    ...
    var detector = try builtin.Detector.init(t.allocator, opts);
    defer detector.deinit(t.allocator);
    const Kind = @import("engine_test").core.native_detection_record.Kind;
    const cases = .{
