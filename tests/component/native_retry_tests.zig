...
const native_config = @import("engine_test").config.native;
const retry_config = @import("engine_test").config.native_retry_policy;
...
test "native retry: committed processing floor preserves receipts and pauses admission until catch-up" {
    ...
    const processing = @import("engine_test").core.record_pipeline;
    const Stub = struct {
        fn prepare(_: @import("engine_test").core.source_record.Record, _: ?*anyopaque) !processing.Prepared {
