...
const reload = @import("engine_test").runtime.native_reload;
...
test "native reload: digests and generation identities are deterministic and clock-distinct" {
    ...
    const policy = @import("engine_test").core.native_retry.Policy{ .maxretry = 3, .window_us = 1_000_000, .duration = .{ .finite_us = 1_000_000 }, .max_subjects = 4 };
    const g = [_]u8{1} ** 32;
