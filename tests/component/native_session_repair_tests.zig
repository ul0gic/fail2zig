...
const journal = @import("engine_test").core.native_journal_session;
...
test "native session repair: exact committed pending proof permits fresh cursor recovery without resetting episode" {
    ...
    _ = try source.poll(@import("engine_test").core.record_pipeline.Pipeline.acknowledge, &old.pipe);
...
const GenerationProbe = struct {
    calls: usize = 0,
    admits: usize = 0,
    const detected = @import("engine_test").core.native_detection_record;
    ...
    fn manifest(_: []const u8, _: [32]u8, ctx: ?*anyopaque) !@import("engine_test").core.native_consumer.Manifest {
        const self: *@This() = @ptrCast(@alignCast(ctx.?));
        self.calls += 1;
        return error.UnexpectedPreflightCallback;
    }
...
test "native session repair: pure generation equals file and journal builtin custom retry constructors" {
    for ([_]bool{ false, true }) |is_journal| for ([_]bool{ false, true }) |custom| for ([_]bool{ false, true }) |with_retry| {
        ...
        try f.store.enableEffects();
        try f.store.enableConsumerManifests();
        var detector = try @import("engine_test").core.native_builtin_detector.Detector.init(t.allocator, .{ .filter = "sshd", .body = .whole, .ignore_capacity = 8, .max_decoded_bytes = 2048 });
        defer detector.deinit(t.allocator);
        const profile = try @import("engine_test").core.native_journal_origin.Profile.init("0123456789abcdef0123456789abcdef", &.{"/fixture/sshd"});
        const qualified = try @import("engine_test").core.native_journal_detector.Detector.init(&detector, profile);
        var clock = Clock{};
        var mock = Mock{ .failure = error.UnexpectedSourceIo };
        var probe = GenerationProbe{};
        var scratch: [2048]u8 = undefined;
        const retry: ?@import("engine_test").core.native_retry.Policy = if (with_retry) .{ .maxretry = 2, .window_us = 600_000_000, .duration = .{ .finite_us = 60_000_000 }, .max_subjects = 8 } else null;
